...
float UltrasonicSensor::measureDistance() {
    ...
    auto timeout = std::chrono::milliseconds(60); 
    while (!echoStart) {
        bool value;
        ...
        if (std::chrono::high_resolution_clock::now() - start > timeout) {
            ...
            return -1.0f; // Return -1 to indicate failure
        }
    }
    ...
    while (true) {
        ...
        if (std::chrono::high_resolution_clock::now() - start > timeout) {
            std::cerr << "Timeout waiting for echo stop" << std::endl;
            return -1.0f; // Return -1 to indicate failure
        }
    }
    ...
    if (distance < 0 || distance > 300) {
        std::cerr << "Measured distance out of range: " << distance << " cm" << std::endl;
        return -1.0f; // Return -1 for invalid distances
    }
