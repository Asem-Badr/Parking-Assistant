...
class SteeringWheelViewInverted @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val baseY = 0f // Start at the top of the view
        ...
        val leftPath = Path().apply {
            moveTo(centerX - lineSpacing, baseY) // Start at the top-left line
            quadTo(
                centerX - lineSpacing - leftCurveOffset, baseY + lineLength / 2, // Curve midpoint
                centerX - lineSpacing, baseY + lineLength // End at the bottom-left line
            )
        }

        val rightPath = Path().apply {
            moveTo(centerX + lineSpacing, baseY) // Start at the top-right line
            quadTo(
                centerX + lineSpacing + rightCurveOffset, baseY + lineLength / 2, // Curve midpoint
                centerX + lineSpacing, baseY + lineLength // End at the bottom-right line
            )
        }
        ...
        val shadowPath = Path().apply {
            addPath(leftPath)
            lineTo(centerX + lineSpacing, baseY + lineLength) // Connect to the right curve bottom
            quadTo(
                centerX + lineSpacing + rightCurveOffset, baseY + lineLength / 2, // Right curve midpoint
                centerX + lineSpacing, baseY // Back to the top-right
            )
